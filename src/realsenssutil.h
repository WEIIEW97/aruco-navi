...
#ifndef BLIND_ASSIST_REALSENSSUTIL_H
#define BLIND_ASSIST_REALSENSSUTIL_H
...
#endif // BLIND_ASSIST_REALSENSSUTIL_H

// Find devices with specified streams
bool device_with_streams(std::vector<rs2_stream> stream_requests,
  ...
  for (auto& type : unavailable_streams) {
    ...
    case RS2_STREAM_COLOR:
      std::cerr << "The demo requires Realsense camera with RGB sensor"
                << std::endl;
      break;
    default:
      throw std::runtime_error(
          "The requested stream: " + std::to_string(type) +
          ", for the demo is not supported by connected devices!"); // stream
                                                                    // type
    }
  }
  return false;
...
struct tile_properties {
  ...
  Priority priority; // when should the tile be drawn?: high priority is on top
                     // of all, medium is a layer under top layer, low is a
                     // layer under medium layer
};
  ...
  void draw_motion(const rs2::motion_frame& f, const rect& r) {
    ...
    if (norm < vec_threshold) {
      ...
      s1 << "(" << std::fixed << std::setprecision(precision) << x << ","
         << std::fixed << std::setprecision(precision) << y << "," << std::fixed
      ...
      print_text_in_3d(x / 2, y / 2, z / 2, s2.str().c_str(), true, model, proj,
                       1 / norm);
    }
    glMatrixMode(GL_PROJECTION);
    glPopMatrix();
...
public:
  ...
  void render(const rs2::frame& frame, const rect& rect, float alpha = 1.f) {
    if (auto vf = frame.as<rs2::video_frame>()) {
      ...
      throw std::runtime_error("Rendering is currently supported for video, "
                               "motion and pose frames only");
  }
...
public:
  ...
  window(int width, int height, const char* title)
      : _width(width), _height(height), _canvas_left_top_x(0),
        _canvas_left_top_y(0), _canvas_width(width), _canvas_height(height) {
    glfwInit();
  ...
  // another c'tor for adjusting specific frames in specific tiles, this window
  // is NOT resizeable
  window(unsigned width, unsigned height, const char* title,
         unsigned tiles_in_row, unsigned tiles_in_col,
         float canvas_width = 0.8f, float canvas_height = 0.6f,
         float canvas_left_top_x = 0.1f, float canvas_left_top_y = 0.075f)
      : _width(width), _height(height), _tiles_in_row(tiles_in_row),
        _tiles_in_col(tiles_in_col)

  {
    // user input verification for mosaic size, if invalid values were given -
    // set to default
    if (canvas_width < 0 || canvas_width > 1 || canvas_height < 0 ||
        canvas_height > 1 || canvas_left_top_x < 0 || canvas_left_top_x > 1 ||
        canvas_left_top_y < 0 || canvas_left_top_y > 1) {
      std::cout << "Invalid window's size parameter entered, setting to "
                   "default values"
                << std::endl;
      canvas_width = 0.8f;
      canvas_height = 0.6f;
  ...
  // gets as argument a map of the -need to be drawn- frames with their tiles
  // properties, which indicates where and what size should the frame be drawn
  // on the canvas
  void show(const frames_mosaic& frames) {
    ...
    if (frames.size()) {
      ...
      std::sort(vector_frames.begin(), vector_frames.end(),
                [](const frame_and_tile_property& frame1,
                   const frame_and_tile_property& frame2) {
      ...
      // iterate over frames in ascending priority order (so that lower priority
      // frame is drawn first, and can be over-written by higher priority frame
      // )
      for (const auto& frame : vector_frames) {
        ...
        rect viewport_loc{_tile_width_pixels * attr.x + _canvas_left_top_x,
                          ...
                          _tile_width_pixels * attr.w *
                              frame_width_size_from_tile_width,
      ...
      _main_win.put_text(
          "Connect one or more Intel RealSense devices and rerun the example",
          0.3f, 0.5f,
          {float(_canvas_left_top_x), float(_canvas_left_top_y),
           float(_canvas_width), float(_canvas_height)});
    }
  }

  /////////////////////////////////////////////////////////////
  //     get_pos_on_current_image:
  // There may be several windows displayed on the sceen, as described in the
  // frames_mosaic structure. The windows are displayed in a reduced resolution,
  // appropriate the amount of space allocated for them on the screen. This
  // function converts from screen pixel to original image pixel.
  //
  // Input:
  // pos - pixel in screen coordinates.
  // frames - structure of separate windows displayed on screen.
  // Returns:
  // The index of the window the screen pixel is in and the pixel in that window
  // in the original window's resolution.
  frame_pixel get_pos_on_current_image(float2 pos,
                                       const frames_mosaic& frames) {
      ...
      if (auto vf = frame.second.first.as<rs2::video_frame>()) {
        ...
        rect viewport_loc{_tile_width_pixels * attr.x + _canvas_left_top_x,
                          ...
                          _tile_width_pixels * attr.w *
                              frame_width_size_from_tile_width,
                          _tile_height_pixels * attr.h};
        viewport_loc = viewport_loc.adjust_ratio(
            {(float)vf.get_width(), (float)vf.get_height()});
        if (pos.x >= viewport_loc.x &&
            pos.x < viewport_loc.x + viewport_loc.w &&
            pos.y >= _height - (viewport_loc.y + viewport_loc.h) &&
            pos.y < _height - viewport_loc.y) {
          float image_rect_ratio =
              (float)vf.get_width() /
              viewport_loc.w; // Ratio for y-axis is the same.
...
private:
  ...
  rect calc_grid(rect r, size_t streams) {
    if (r.w <= 0 || r.h <= 0 || streams <= 0)
      throw std::runtime_error("invalid window configuration request, failed "
                               "to calculate window grid");
    float ratio = r.w / r.h;
...
struct window_key_listener {
  ...
  window_key_listener(window& win) {
    win.on_key_release = std::bind(&window_key_listener::on_key_release, this,
                                   std::placeholders::_1);
  }
...
void draw_pointcloud(float width, float height, glfw_state& app_state,
  ...
  for (int i = 0; i < points.size(); i++) {
    if (vertices[i].z) {
      // upload the point and texture coordinates only for points we have depth
      // data for
...
void get_screen_resolution(unsigned int& window_width,
